...
enum class CAMERA_TYPE_Enum : uint8_t { CAMERA_TYPE_PERSPECTIVE = 0, CAMERA_TYPE_ORTHOGRAPHIC, CAMERA_TYPE_Count };

enum class CAMERA_MODE_Enum : uint8_t { CAMERA_MODE_TURNTABLE_Y = 0, CAMERA_MODE_FREE, CAMERA_MODE_Count };
...
public:
    ...
    void SetPerspectiveLimitZ(const float& vNearPlane, const float& vFarPlane);
    void SetOrthographicLimitZ(const float& vLeftPlane, const float& vRightPlane, const float& vBottomPlane, const float& vTopPlane);
    ...
    void SetTranslation(const glm::vec3& vTranslation) override;
    ...
    const CameraSettingsStruct& GetCameraSettings() const {
