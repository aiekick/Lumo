...
struct LUMO_BACKEND_API GraphStyleStruct {
    // node
...
struct LUMO_BACKEND_API OldFuncToChangeStruct {
...
struct LUMO_BACKEND_API Func_Loc_In_Code_Struct {
    std::string parentFunc;
    std::string stamp;
    std::string sourceFunc;
...
enum class LINK_TYPE_Enum : uint8_t { LINK_TYPE_LINE = 0, LINK_TYPE_SPLINE, LINK_TYPE_STRAIGHT, LINK_TYPE_Count };

/// <summary>
/// fixed mean : slots count are fixed and defined by the node
/// dynamic mean : slots count a varaible
/// </summary>
enum class NODE_INTERNAL_MODE_Enum : uint8_t { NODE_INTERNAL_MODE_FIXED = 0, NODE_INTERNAL_MODE_DYNAMIC };

struct LUMO_BACKEND_API BaseNodeState {
...
class LUMO_BACKEND_API BaseNode : public conf::ConfigAbstract,
                                  public GuiInterface,
...
public:
    typedef std::function<void(const BaseNodeWeak&)> BaseNodeFunctor;
    typedef std::function<void(const NodeSlotWeak&, const ImGuiMouseButton&)> NodeSlotImGuiMouseButtonFunctor;
    typedef std::function<void(const std::string&)> StringFunctor;
    typedef std::function<BaseNodeWeak(BaseNodeWeak vNodeGraph, BaseNodeState* vBaseNodeState)> BaseNodeStateFunctor;
    typedef std::function<bool(
        BaseNodeWeak, tinyxml2::XMLElement*, tinyxml2::XMLElement*, const std::string&, const std::string&, const ct::fvec2&, const size_t&)>
        BaseNodeXmlLoadingFunctor;

public:
    BaseNodeFunctor m_OpenGraphCallback;                             // open graph
    NodeSlotImGuiMouseButtonFunctor m_SelectSlotCallback;            // select slot
    NodeSlotImGuiMouseButtonFunctor m_SelectForGraphOutputCallback;  // select for graph output
    StringFunctor m_OpenCodeCallback;                                // open code
    StringFunctor m_LogErrorsCallback;                               // log errors
    StringFunctor m_LogInfosCallback;                                // log infos
    BaseNodeFunctor m_SelectNodeCallback;                            // select node
    BaseNodeStateFunctor m_ShowNewNodeMenuCallback;                  // new node menu
    BaseNodeXmlLoadingFunctor m_LoadNodeFromXMLCallback;             // log infos

public:
    void SetOpenGraphCallback(BaseNodeFunctor vBaseNodeFunctor);
    void OpenGraph_Callback(const BaseNodeWeak& vNode);

    void SetSelectSlotCallback(NodeSlotImGuiMouseButtonFunctor vSelectSlotCallback);
    void SelectSlot_Callback(const NodeSlotWeak& vSlot, const ImGuiMouseButton& vMouseButton);

    void SetSelectForGraphOutputCallback(NodeSlotImGuiMouseButtonFunctor vSelectForGraphOutputCallback);
    void SelectForGraphOutput_Callback(const NodeSlotWeak& vSlot, const ImGuiMouseButton& vMouseButton);

    void SetOpenCodeCallback(StringFunctor vOpenCodeCallback);
    void OpenCode_Callback(const std::string& vCode);

    void SetLogErrorsCallback(StringFunctor vLogErrorsCallback);
    void LogErrors_Callback(const std::string& vErrors);

    void SetLogInfosCallback(StringFunctor vLogInfosCallback);
    void LogInfos_Callback(const std::string& vInfos);

    void SetSelectNodeCallback(BaseNodeFunctor vSelectNodeCallback);
    void SelectNode_Callback(const BaseNodeWeak& vNode);

    void SetNewNodeMenuCallback(BaseNodeStateFunctor vShowNewNodeMenuCallback);
    void ShowNewNodeMenu_Callback(const BaseNodeWeak& vNodeGraph, BaseNodeState* vBaseNodeState);

    void SetLoadNodeFromXMLCallback(BaseNodeXmlLoadingFunctor vLoadNodeFromXMLCallback);
    bool LoadNodeFromXML_Callback(const BaseNodeWeak& vBaseNodeWeak,
