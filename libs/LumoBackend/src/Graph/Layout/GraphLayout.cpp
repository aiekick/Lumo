...
void ColumnContainerStruct::AddNode(BaseNodeWeak vNode) {
    if (!vNode.expired()) {
        ...
        if (ptr) {
            if (ptr->inserted) {
                LogVarDebugError("le node a deja �t� ins�r�.. (ColumnContainerStruct::AddNode)");
            }
        ...
        for (auto &nodePair : column.nodes) {
            ...
            if (!node.expired()) {
                ...
                if (nodePtr) {
                    ...
                    nodePtr->pos.x =
                        column.offset.x + column.size.x * s_NodeCentering - nodePtr->size.x * s_NodeCentering;  // centrage x du node dans la colonne
