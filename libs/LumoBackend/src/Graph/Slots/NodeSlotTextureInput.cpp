...
void NodeSlotTextureInput::OnConnectEvent(NodeSlotWeak vOtherSlot)
{
	if (slotType == "TEXTURE_2D")
	{
		...
		if (endSlotPtr)
		{
			auto parentNodePtr = dynamic_pointer_cast<TextureInputInterface<0u>>(parentNode.lock());
			if (parentNodePtr)
			{
				auto otherTextureNodePtr = dynamic_pointer_cast<TextureOutputInterface>(endSlotPtr->parentNode.lock());
		...
		if (emiterSlotPtr)
		{
			if (emiterSlotPtr->IsAnOutput())
			{
				...
				if (parentNodePtr)
				{
					...
					{
						auto otherNodePtr = dynamic_pointer_cast<TextureOutputInterface>(emiterSlotPtr->parentNode.lock());
						if (otherNodePtr)
						{
