...
NodeSlotStorageBufferOutputPtr NodeSlotStorageBufferOutput::Create(
    const std::string& vName, const std::string& vType, const uint32_t& vBindingPoint) {
...
NodeSlotStorageBufferOutput::NodeSlotStorageBufferOutput() : NodeSlotOutput("", "") {
    pinID = sGetNewSlotId();
    color = sGetSlotColors()->GetSlotColor(slotType);
    colorIsSet = true;
}

NodeSlotStorageBufferOutput::NodeSlotStorageBufferOutput(const std::string& vName, const std::string& vType) : NodeSlotOutput(vName, vType) {
    pinID = sGetNewSlotId();
    color = sGetSlotColors()->GetSlotColor(slotType);
    colorIsSet = true;
}
