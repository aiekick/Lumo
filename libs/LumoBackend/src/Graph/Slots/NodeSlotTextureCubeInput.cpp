...
NodeSlotTextureCubeInputPtr NodeSlotTextureCubeInput::Create(
...
NodeSlotTextureCubeInput::NodeSlotTextureCubeInput() : NodeSlotInput("", "TEXTURE_CUBE") {
    pinID = sGetNewSlotId();
    color = sGetSlotColors()->GetSlotColor(slotType);
    colorIsSet = true;
}

NodeSlotTextureCubeInput::NodeSlotTextureCubeInput(const std::string& vName, const uint32_t& vBindingPoint) : NodeSlotInput(vName, "TEXTURE_CUBE") {
    pinID = sGetNewSlotId();
    color = sGetSlotColors()->GetSlotColor(slotType);
    colorIsSet = true;
...
void NodeSlotTextureCubeInput::TreatNotification(const NotifyEvent& vEvent, const NodeSlotWeak& vEmitterSlot, const NodeSlotWeak& vReceiverSlot) {
    if (vEvent == TextureUpdateDone) {
        ...
        if (emiterSlotPtr) {
            if (emiterSlotPtr->IsAnOutput()) {
                auto parentNodePtr = parentNode.lock();
                if (parentNodePtr) {
                    auto parentTexture2DInputNodePtr = dynamic_pointer_cast<TextureCubeInputInterface<0u>>(parentNodePtr);
                    if (parentTexture2DInputNodePtr) {
                        ...
                        if (otherNodePtr) {
                            ...
                            if (receiverSlotPtr) {
                                ...
                                parentTexture2DInputNodePtr->SetTextureCube(receiverSlotPtr->descriptorBinding, descPtr, &textureSize);
                            }
                        } else {
                            CTOOL_DEBUG_BREAK;
                        }
