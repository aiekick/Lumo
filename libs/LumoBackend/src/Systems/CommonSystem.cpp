...
void CommonSystem::SetTranslation(const glm::vec3& /*vTranslation*/) {
    ZoneScoped;
    CTOOL_DEBUG_BREAK;
}
...
void CommonSystem::IncRotateXYZ(const ct::fvec3& vMouseOffset) {
    ...
    if (!m_CameraSettings.m_RotationXYZLock.x) {
        m_CameraSettings.m_RotationXYZ.x += mouseOffset.x * m_CameraSettings.m_RotationFactor;
    ...
    if (!m_CameraSettings.m_RotationXYZLock.y) {
        m_CameraSettings.m_RotationXYZ.y += mouseOffset.y * m_CameraSettings.m_RotationFactor;
    ...
    if (!m_CameraSettings.m_RotationXYZLock.z) {
        m_CameraSettings.m_RotationXYZ.z += mouseOffset.z * m_CameraSettings.m_RotationFactor;
...
bool CommonSystem::DrawImGui() {
    ...
    if (ImGui::CollapsingHeader("Camera")) {
        ...
        if (ImGui::RadioButtonLabeled(
                aw, "Perspective", "Use Perspective Camera", m_CameraSettings.m_CameraType == CAMERA_TYPE_Enum::CAMERA_TYPE_PERSPECTIVE)) {
        ...
        if (ImGui::RadioButtonLabeled(
                aw, "Orthographic", "Use Orthographic Camera", m_CameraSettings.m_CameraType == CAMERA_TYPE_Enum::CAMERA_TYPE_ORTHOGRAPHIC)) {
        ...
        if (ImGui::RadioButtonLabeled(
                aw, "Turntable Y", "Turntable Y Mode", m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y)) {
        ...
        if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_FREE) {
            ...
            change |= ImGui::SliderFloatDefaultCompact(
                -1.0f, "Cam Pos X##campx", &m_CameraSettings.m_PosXYZ.x, -10.0f, 10.0f, s_CameraSettingsStruct_Default.m_PosXYZ.x);
            change |= ImGui::SliderFloatDefaultCompact(
                -1.0f, "Cam Pos Y##campy", &m_CameraSettings.m_PosXYZ.y, -10.0f, 10.0f, s_CameraSettingsStruct_Default.m_PosXYZ.y);
            change |= ImGui::SliderFloatDefaultCompact(
                -1.0f, "Cam Pos Z##campz", &m_CameraSettings.m_PosXYZ.z, -10.0f, 10.0f, s_CameraSettingsStruct_Default.m_PosXYZ.z);
            change |= ImGui::SliderFloatDefaultCompact(
                -1.0f, "Speed##camspeed", &m_CameraSettings.m_SpeedFactor, 0.0f, 100.0f, s_CameraSettingsStruct_Default.m_SpeedFactor);
        }

        if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y) {
            ImGui::FramedGroupSeparator();

            ImGui::CheckBoxIcon("##lock_target_x", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_TargetXYZLock.x);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("lock camera target x\nfrom modification with mouse or gizmo");
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Target X##tgtx", &m_CameraSettings.m_TargetXYZ.x, -10.0f, 10.0f, 0.0f);
            ImGui::CheckBoxIcon("##lock_target_y", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_TargetXYZLock.y);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("lock camera target y\nfrom modification with mouse or gizmo");
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Target Y##tgty", &m_CameraSettings.m_TargetXYZ.y, -10.0f, 10.0f, 0.0f);
            ImGui::CheckBoxIcon("##lock_target_z", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_TargetXYZLock.z);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("lock camera target z\nfrom modification with mouse or gizmo");
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Target Z##tgtz", &m_CameraSettings.m_TargetXYZ.z, -10.0f, 10.0f, 0.0f);

            ImGui::FramedGroupSeparator();

            ImGui::CheckBoxIcon("##lock_trans_x", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_TransXYLock.x);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("lock translation x");
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Trans X##transx", &m_CameraSettings.m_TransXY.x, -10.0f, 10.0f, 0.0f);
            ImGui::CheckBoxIcon("##lock_trans_y", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_TransXYLock.y);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("lock translation y");
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Trans Y##transy", &m_CameraSettings.m_TransXY.y, -10.0f, 10.0f, 0.0f);
            change |= ImGui::SliderFloatDefaultCompact(
                -1.0f, "Factor##trans", &m_CameraSettings.m_TransFactor, 0.0f, 100.0f, s_CameraSettingsStruct_Default.m_TransFactor);
        }

        ImGui::FramedGroupSeparator();

        ImGui::CheckBoxIcon("##lock_rot_x", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_RotationXYZLock.x);
        ImGui::SameLine();
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("lock rotation x\nfrom modification with mouse");
        change |= ImGui::SliderFloatDefaultCompact(-1.0f, "RotX##rotx", &m_CameraSettings.m_RotationXYZ.x, -3.14159f, 3.14159f, 0.0f);
        ImGui::CheckBoxIcon("##lock_rot_y", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_RotationXYZLock.y);
        ImGui::SameLine();
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("lock rotation y\nfrom modification with mouse");
        change |= ImGui::SliderFloatDefaultCompact(-1.0f, "RotY##roty", &m_CameraSettings.m_RotationXYZ.y, -3.14159f, 3.14159f, 0.0f);
        ImGui::CheckBoxIcon("##lock_rot_z", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_RotationXYZLock.z);
        ImGui::SameLine();
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("lock rotation z\nfrom modification with mouse");
        change |= ImGui::SliderFloatDefaultCompact(-1.0f, "RotZ##rotz", &m_CameraSettings.m_RotationXYZ.z, -3.14159f, 3.14159f, 0.0f);
        change |= ImGui::SliderFloatDefaultCompact(
            -1.0f, "Factor##rotxyz", &m_CameraSettings.m_RotationFactor, 0.0f, 6.28318f, s_CameraSettingsStruct_Default.m_RotationFactor);

        // ImGui::FramedGroupSeparator();
        // ImGui::CheckBoxIcon("##lock_uni_scale", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_UniformScaleLock); ImGui::SameLine(); if
        // (ImGui::IsItemHovered()) ImGui::SetTooltip("lock scale"); change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Scale##scale",
        // &m_CameraSettings.m_UniformScale, 0.00001f, 2.0f, 1.0f);

        if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y) {
            ImGui::FramedGroupSeparator();

            ImGui::CheckBoxIcon("##lock_zoom", LUMO_BACKEND_CAMERA_ICON_LABEL_LOCK, &m_CameraSettings.m_ZoomLock);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("lock zoom");
            change |= ImGui::SliderFloatDefaultCompact(
                -1.0f, "Zoom##Zoom", &m_CameraSettings.m_Zoom, 0.00001f, 100.0f, s_CameraSettingsStruct_Default.m_Zoom, 0.0f, "%.5f");
            change |= ImGui::SliderFloatDefaultCompact(
                -1.0f, "Factor##Zoom", &m_CameraSettings.m_ZoomFactor, 0.0f, 100.0f, s_CameraSettingsStruct_Default.m_ZoomFactor);
        }
        ...
        if (VRBackend::Instance()->IsProjectLoaded()) {
            ...
            ImGui::CheckBoxBoolDefault("Use VR Controller for control Camera", &m_CameraSettings.m_UseVRControllerForControlCamera,
                ...
                "left or right Squeeze are pressed");
            if (m_CameraSettings.m_UseVRControllerForControlCamera) {
                change |= ImGui::SliderFloatDefaultCompact(-1.0f, "VR Step Rot X##vrrotx", &m_CameraSettings.m_VRRotStepFactor.x, 0.00001f, 2.0f,
                    s_CameraSettingsStruct_Default.m_VRRotStepFactor.x, 0.0f, "%.5f");
                change |= ImGui::SliderFloatDefaultCompact(-1.0f, "VR Step Rot Y##vrroty", &m_CameraSettings.m_VRRotStepFactor.y, 0.00001f, 2.0f,
                    s_CameraSettingsStruct_Default.m_VRRotStepFactor.y, 0.0f, "%.5f");
                change |= ImGui::SliderFloatDefaultCompact(-1.0f, "VR Step Rot Z##vrrotz", &m_CameraSettings.m_VRRotStepFactor.z, 0.00001f, 2.0f,
                    s_CameraSettingsStruct_Default.m_VRRotStepFactor.z, 0.0f, "%.5f");
                change |= ImGui::SliderFloatDefaultCompact(-1.0f, "VR Step Zoom##vrzoom", &m_CameraSettings.m_VRZoomStepFactor, 0.00001f, 5.0f,
                    s_CameraSettingsStruct_Default.m_VRZoomStepFactor, 0.0f, "%.5f");
            }
        }
#endif
...
glm::mat4 CommonSystem::ComputeModelMatrix() {
    ...
    if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y) {
        ...
        Model = glm::translate(
            glm::mat4_cast(camOrientation), glm::vec3(m_CameraSettings.m_PosXYZ.x, m_CameraSettings.m_PosXYZ.y, m_CameraSettings.m_PosXYZ.z));
    }
...
bool CommonSystem::ComputeCameras(const ct::uvec2& vScreenSize) {
    ...
    if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y) {
        ...
        glm::quat q = glm::quat(glm::vec3(m_CameraSettings.m_RotationXYZ.x, m_CameraSettings.m_RotationXYZ.y, m_CameraSettings.m_RotationXYZ.z)); //
        pitch, yaw, roll
