...
{
	ImGui::SliderFloatDefaultCompact(200.0f, "Contrast Ratio", &CustomStyle::Instance()->puContrastRatio, 0.0f, 21.0f, 3.0f, 0.0f);
...
bool ImGui::ColorPicker3DefaultForNode(float vWidth, const char* vName, float* vCol, float* vDefault)
{
	bool change = false;
	...
	ImGui::PushID(ImGui::IncPUSHID());
	ImGui::BeginGroup();
	change = ImGui::ContrastedButton(ICON_NDP_RESET, nullptr, nullptr, vWidth);
	if (change)
	{
		vCol[0] = vDefault[0];
		vCol[1] = vDefault[1];
	...
	change |= ImGui::ColorPicker4ForNode(vWidth, vName, vCol);
	ImGui::EndGroup();
	ImGui::PopID();
	*/
...
bool ImGui::ColorPicker4DefaultForNode(float vWidth, const char* vName, float* vCol, float* vDefault)
{
	bool change = false;
...
static void ImWidgets_ColorEditRestoreHS(const float* col, float* H, float* S, float* V)
{
...
static void ImWidgets_RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha)
{
...
bool ImGui::ColorPicker4ForNode(float vWidth, const char* label, float* col)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
	...
	const float width = vWidth;// CalcItemWidth();
	...
	float square_sz = GetFrameHeight() * 0.25f;
	...
	//if (flags & ImGuiColorEditFlags_InputRGB)
	{
		...
		ImWidgets_ColorEditRestoreHS(col, &H, &S, &V);
	}
	...
	if (value_changed)
	{
		//if (flags & ImGuiColorEditFlags_InputRGB)
		{
			...
			B = col[2];
			ColorConvertRGBtoHSV(R, G, B, H, S, V);
			ImWidgets_ColorEditRestoreHS(col, &H, &S, &V);   // Fix local Hue as display below will use it immediately.
		}
		/*else if (flags & ImGuiColorEditFlags_InputHSV)
		{
			H = col[0];
	...
	else *///if (flags & ImGuiColorEditFlags_PickerHueBar)
	{
		...
		ImWidgets_RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, style.Alpha);
	}

	// Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
	float sv_cursor_rad = value_changed_sv ? 5.0f : 2.5f;
	draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 24);
	draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad + 1, col_midgrey, 24);
	draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad, col_white, 24);
	...
	if (alpha_bar)
	{
		...
		ImWidgets_RenderArrowsForVerticalBar(draw_list, ImVec2(bar1_pos_x - 1, bar1_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, style.Alpha);
	}
