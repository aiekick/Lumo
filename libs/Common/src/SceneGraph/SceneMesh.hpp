...
typedef ct::cWeak<SceneMesh<VertexStruct::P3_N3_TA3_BTA3_T2_C4>> SceneMeshWeak;
...
public:
	...
	static std::shared_ptr<SceneMesh<T_VertexType>> Create(vkApi::VulkanCorePtr vVulkanCorePtr, std::vector<T_VertexType> vVerticeArray, IndiceArray vIndiceArray);
	static std::shared_ptr<SceneMesh<T_VertexType>> Copy(const ct::cWeak<SceneMesh<T_VertexType>>& vSceneMeshToCopy);
...
private:
	ct::cWeak<SceneMesh<VertexStruct::P3_N3_TA3_BTA3_T2_C4>> m_This;
...
public:
	...
	SceneMesh(vkApi::VulkanCorePtr vVulkanCorePtr, std::vector<T_VertexType> vVerticeArray, IndiceArray vIndiceArray);
	~SceneMesh();
...
bool SceneMesh<T_VertexType>::BuildVBO(bool vUseSBO)
{
	...
	m_Vertices.m_Buffer = vkApi::VulkanRessource::createVertexBufferObject(m_VulkanCorePtr, m_Vertices.m_Array, vUseSBO, false, m_VulkanCorePtr->GetSupportedFeatures().is_RTX_Supported);
...
void SceneMesh<T_VertexType>::BuildIBO(bool vUseSBO)
{
	...
	if (devicePtr)
	{
		m_Indices.m_Buffer = vkApi::VulkanRessource::createIndexBufferObject(m_VulkanCorePtr, m_Indices.m_Array, vUseSBO, false, devicePtr->GetRTXUse()); // the last true is for RTX
