...
std::shared_ptr<SpecularModule> SpecularModule::Create(vkApi::VulkanCore* vVulkanCore)
{
...
SpecularModule::SpecularModule(vkApi::VulkanCore* vVulkanCore)
...
bool SpecularModule::Init()
{
	...
	{
		m_SpecularModule_Pass_Ptr = std::make_shared<SpecularModule_Pass>(m_VulkanCore);
		if (m_SpecularModule_Pass_Ptr)
		{
			if (m_SpecularModule_Pass_Ptr->InitCompute2D(map_size, 1U, false, vk::Format::eR32G32B32A32Sfloat))
			{
...
bool SpecularModule::Execute(const uint32_t& vCurrentFrame, vk::CommandBuffer* vCmd)
{
	ZoneScoped;
