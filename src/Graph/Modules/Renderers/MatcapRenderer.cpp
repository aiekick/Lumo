...
#include <Graph/Modules/Renderers/Pass/MatcapRenderer_Mesh_Pass.h>
...
MatcapRenderer::MatcapRenderer(GaiApi::VulkanCoreWeak vVulkanCore) : TaskRenderer(vVulkanCore) {
    ZoneScoped;
...
bool MatcapRenderer::Init() {
    ...
    if (TaskRenderer::InitPixel(map_size)) {
        ...
        if (m_MatcapRenderer_Mesh_Pass_Ptr) {
            if (m_MatcapRenderer_Mesh_Pass_Ptr->InitPixel(
...
bool MatcapRenderer::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    assert(vContextPtr);
...
bool MatcapRenderer::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, void* vUserDatas) {
    assert(vContextPtr);
    ImGui::SetCurrentContext(vContextPtr);
...
void MatcapRenderer::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize, void* vUserDatas) {
    if (m_MatcapRenderer_Mesh_Pass_Ptr) {
        return m_MatcapRenderer_Mesh_Pass_Ptr->SetTexture(vBindingPoint, vImageInfo, vTextureSize, vUserDatas);
    }
}

vk::DescriptorImageInfo* MatcapRenderer::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize, void* vUserDatas) {
    if (m_MatcapRenderer_Mesh_Pass_Ptr) {
        return m_MatcapRenderer_Mesh_Pass_Ptr->GetDescriptorImageInfo(vBindingPoint, vOutSize, vUserDatas);
    }
