...
HeatmapRenderer_Mesh_Pass::HeatmapRenderer_Mesh_Pass(GaiApi::VulkanCoreWeak vVulkanCore) : ShaderPass(vVulkanCore) {
...
bool HeatmapRenderer_Mesh_Pass::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    assert(vContextPtr);
    ...
    ImGui::SameLine();
    if (ImGui::RadioButtonLabeled(0.0f, "R", m_UBOFrag.channel_idx == 0, false)) {
        change = true;
        m_UBOFrag.channel_idx = 0;
    }
    ImGui::SameLine();
    if (ImGui::RadioButtonLabeled(0.0f, "G", m_UBOFrag.channel_idx == 1, false)) {
        change = true;
        m_UBOFrag.channel_idx = 1;
    }
    ImGui::SameLine();
    if (ImGui::RadioButtonLabeled(0.0f, "B", m_UBOFrag.channel_idx == 2, false)) {
        change = true;
        m_UBOFrag.channel_idx = 2;
    }
    ImGui::SameLine();
    if (ImGui::RadioButtonLabeled(0.0f, "A", m_UBOFrag.channel_idx == 3, false)) {
        change = true;
        m_UBOFrag.channel_idx = 3;
    }
...
bool HeatmapRenderer_Mesh_Pass::CreateSBO() {
    ...
    m_SBO_Colors = VulkanRessource::createStorageBufferObject(
        m_VulkanCore, sizeInBytes, VmaMemoryUsage::VMA_MEMORY_USAGE_CPU_TO_GPU, "HeatmapRenderer_Mesh_Pass");
...
bool HeatmapRenderer_Mesh_Pass::CreateUBO() {
    ...
    m_UBOVertPtr = VulkanRessource::createUniformBufferObject(m_VulkanCore, sizeof(UBOVert), "HeatmapRenderer_Mesh_Pass");
    if (m_UBOVertPtr) {
    ...
    m_UBOFragPtr = VulkanRessource::createUniformBufferObject(m_VulkanCore, sizeof(UBOFrag), "HeatmapRenderer_Mesh_Pass");
    if (m_UBOFragPtr) {
