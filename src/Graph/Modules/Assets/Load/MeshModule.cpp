...
bool MeshModule::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    assert(vContextPtr);
    ...
    if (ImGui::CollapsingHeader("Model")) {
        ...
        if (!m_SceneModelPtr->empty()) {
            ...
            for (const auto& meshPtr : *m_SceneModelPtr) {
                if (meshPtr) {
                    ImGui::Text("Mesh %u : %s %s %s %s %s %s", idx++, meshPtr->HasNormals() ? "N" : " ", meshPtr->HasTangeants() ? "Tan" : " ",
                        meshPtr->HasBiTangeants() ? "BTan" : " ", meshPtr->HasTextureCoords() ? "UV" : " ", meshPtr->HasVertexColors() ? "Col" : " ",
...
bool MeshModule::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, void* vUserDatas) {
    assert(vContextPtr);
    ImGui::SetCurrentContext(vContextPtr);
...
void MeshModule::LoadMesh(const std::string& vFilePathName) {
    if (!vFilePathName.empty()) {
        ...
        try {
            const aiScene* scene = aiImportFile(m_FilePathName.c_str(), aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals |
                    ...
                    if (mesh) {
                        ...
                        if (sceneMeshPtr) {
                            ...
                            for (size_t i = 0; i != mesh->mNumVertices; ++i) {
                                ...
                                if (mesh->mTextureCoords[0] && mesh->mNumUVComponents[0] == 2U) {
