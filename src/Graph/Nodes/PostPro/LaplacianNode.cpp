...
{
	m_NodeType = NodeTypeEnum::LAPLACIAN;
}
...
bool LaplacianNode::Execute(const uint32_t& vCurrentFrame, vk::CommandBuffer* vCmd)
{
	...
	if (m_LaplacianModulePtr)
	{
		return m_LaplacianModulePtr->Execute(vCurrentFrame, vCmd);
	}
	return false;
}
...
void LaplacianNode::DisplayInfosOnTopOfTheNode(BaseNodeStateStruct* vCanvasState)
{
...
void LaplacianNode::NeedResize(ct::ivec2* vNewSize, const uint32_t* vCountColorBuffer)
{
	if (m_LaplacianModulePtr)
	{
		m_LaplacianModulePtr->NeedResize(vNewSize, vCountColorBuffer);
	}
...
void LaplacianNode::JustConnectedBySlots(NodeSlotWeak vStartSlot, NodeSlotWeak vEndSlot)
{
	auto startSlotPtr = vStartSlot.getValidShared();
	auto endSlotPtr = vEndSlot.getValidShared();
	if (startSlotPtr && endSlotPtr && m_LaplacianModulePtr)
	{
...
// le start est toujours le slot de ce node, l'autre le slot du node connecté
void LaplacianNode::JustDisConnectedBySlots(NodeSlotWeak vStartSlot, NodeSlotWeak vEndSlot)
{
	auto startSlotPtr = vStartSlot.getValidShared();
	auto endSlotPtr = vEndSlot.getValidShared();
	if (startSlotPtr && endSlotPtr && m_LaplacianModulePtr)
...
void LaplacianNode::SetTexture(const uint32_t& vBinding, vk::DescriptorImageInfo* vImageInfo)
{
	if (m_LaplacianModulePtr)
	{
		m_LaplacianModulePtr->SetTexture(vBinding, vImageInfo);
	}
}
...
void LaplacianNode::Notify(const NotifyEvent& vEvent, const NodeSlotWeak& vEmmiterSlot, const NodeSlotWeak& vReceiverSlot)
{
	switch (vEvent)
	{
