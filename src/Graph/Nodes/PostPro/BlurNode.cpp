...
std::shared_ptr<BlurNode> BlurNode::Create(vkApi::VulkanCore* vVulkanCore)
{
...
bool BlurNode::Init(vkApi::VulkanCore* vVulkanCore)
{
	name = "Blur";
...
bool BlurNode::Execute(const uint32_t& vCurrentFrame, vk::CommandBuffer* vCmd)
{
	...
	if (m_BlurModulePtr)
	{
		return m_BlurModulePtr->Execute(vCurrentFrame, vCmd);
	}
	return false;
}
...
void BlurNode::DisplayInfosOnTopOfTheNode(BaseNodeStateStruct* vCanvasState)
{
...
void BlurNode::NeedResize(ct::ivec2* vNewSize, const uint32_t* vCountColorBuffer)
{
	if (m_BlurModulePtr)
	{
		m_BlurModulePtr->NeedResize(vNewSize, vCountColorBuffer);
	}
...
void BlurNode::SetTexture(const uint32_t& vBinding, vk::DescriptorImageInfo* vImageInfo)
{
	if (m_BlurModulePtr)
	{
		m_BlurModulePtr->SetTexture(vBinding, vImageInfo);
	}
}
...
void BlurNode::Notify(const NotifyEvent& vEvent, const NodeSlotWeak& vEmmiterSlot, const NodeSlotWeak& vReceiverSlot)
{
	switch (vEvent)
	{
