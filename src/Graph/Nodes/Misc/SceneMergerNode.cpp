...
bool SceneMergerNode::DrawDialogsAndPopups(
    ...
    if (m_SceneMergerModulePtr) {
        return m_SceneMergerModulePtr->DrawDialogsAndPopups(vCurrentFrame, vMaxRect, vContextPtr, vUserDatas);
    }
    return false;
}
...
void SceneMergerNode::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize, void* vUserDatas) {
    ZoneScoped;

    if (m_SceneMergerModulePtr) {
        m_SceneMergerModulePtr->SetTexture(vBindingPoint, vImageInfo, vTextureSize, vUserDatas);
    }
}
...
vk::DescriptorImageInfo* SceneMergerNode::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize, void* vUserDatas) {
    ZoneScoped;

    if (m_SceneMergerModulePtr) {
        return m_SceneMergerModulePtr->GetDescriptorImageInfo(vBindingPoint, vOutSize, vUserDatas);
    }
    ...
    if (m_SceneMergerModulePtr) {
        ...
        if (graph_ptr) {
            ...
            for (const auto& slotID : vSlotsToErase) {
                if (m_Inputs.find(slotID) != m_Inputs.end()) {
                    if (slots_to_keep.find(slotID) == slots_to_keep.end()) {
                        ...
                        LogVarDebugWarning("input slot(%u) not destroyed because of 'no destroy' rule during this connect", slotID);
                    }
                } else {
                    CTOOL_DEBUG_BREAK;
            ...
            for (const auto& slot : m_Inputs) {
                if (slot.second && slot.second->linkedSlots.empty())  // if we found at least one empty nide, we will not add a slot
                {
