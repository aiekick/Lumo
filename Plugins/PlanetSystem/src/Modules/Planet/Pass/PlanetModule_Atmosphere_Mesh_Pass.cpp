...
#include "PlanetModule_Atmosphere_Mesh_Pass.h"
...
PlanetModule_Atmosphere_Mesh_Pass::PlanetModule_Atmosphere_Mesh_Pass(GaiApi::VulkanCoreWeak vVulkanCore) : MeshShaderPass<VertexStruct::P3_N3_C4>(vVulkanCore, MeshShaderPassType::PIXEL) {
    ZoneScoped;

    SetRenderDocDebugName("Mesh Pass : Planet Atmosphere", MESH_SHADER_PASS_DEBUG_COLOR);

    // m_DontUseShaderFilesOnDisk = true;
}

PlanetModule_Atmosphere_Mesh_Pass::~PlanetModule_Atmosphere_Mesh_Pass() {
    ZoneScoped;

    Unit();
}

void PlanetModule_Atmosphere_Mesh_Pass::ActionBeforeInit() {
    ZoneScoped;
...
bool PlanetModule_Atmosphere_Mesh_Pass::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    ZoneScoped;
    ...
    if (ImGui::CollapsingHeader("Atmosphere : Mesh Analyze")) {
        need_ubos_update |= ImGui::CheckBoxIntDefault("Use debug##atmosphere", &m_UBO_Frag.u_use_debug, 0);
        if (m_UBO_Frag.u_use_debug) {
            ...
            need_ubos_update |= ImGui::ContrastedComboVectorDefault(0.0f, "Channel##atmosphere", &m_UBO_Frag.u_show_layer, m_Channels, 0);
        ...
        {
            need_ubos_update |= ImGui::CheckBoxIntDefault("Show Shaded Wireframe##atmosphere", &m_UBO_Frag.u_show_shaded_wireframe, 0);
        }

        if (!m_Vertices.m_Array.empty() && !m_Indices.m_Array.empty()) {
            need_ubos_update |= ImGui::CheckBoxBoolDefault("Use Indices Restriction##atmosphere", &m_UseIndiceRestriction, false);
            if (m_UseIndiceRestriction) {
                need_ubos_update |= ImGui::SliderUIntDefaultCompact(0.0f, "count indices##atmosphere", &m_RestrictedIndicesCountToDraw, 0, (uint32_t)m_Indices.m_Array.size(), (uint32_t)m_Indices.m_Array.size());
            }
        }
    ...
    if (ImGui::CollapsingHeader("Atmosphere : Mesh Control")) {
        need_model_update |= ImGui::SliderUIntDefaultCompact(0.0f, "Subdivision Level##atmosphere", &m_SubdivisionLevel, 0U, 5U, 0U);
    }

    if (ImGui::CollapsingHeader("Atmosphere : Mesh Tesselation")) {
        need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Radius##atmosphere", &m_UBO_Tess_Eval.u_radius, 0.0f, 10.0f, 1.0f);
        need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Tesselation level##atmosphere", &m_UBO_Tess_Ctrl.u_tesselation_level, 0.0f, 100.0f, 1.0f);
        need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Displacement Factor##atmosphere", &m_UBO_Tess_Eval.u_displace_factor, 0.0f, 2.0f, 1.0f);
        need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Normal Precision##atmosphere", &m_UBO_Tess_Eval.u_normal_prec, 0.0f, 0.1f, 0.01f);
    }
...
bool PlanetModule_Atmosphere_Mesh_Pass::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    ZoneScoped;
    assert(vContextPtr);
    ImGui::SetCurrentContext(vContextPtr);
    return false;
...
bool PlanetModule_Atmosphere_Mesh_Pass::DrawDialogsAndPopups(const uint32_t& vCurrentFrame, const ImVec2& vMaxSize, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    ZoneScoped;
    assert(vContextPtr);
    ImGui::SetCurrentContext(vContextPtr);
    return false;
...
void PlanetModule_Atmosphere_Mesh_Pass::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize) {
    ZoneScoped;
...
vk::DescriptorImageInfo* PlanetModule_Atmosphere_Mesh_Pass::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize) {
    ZoneScoped;
...
void PlanetModule_Atmosphere_Mesh_Pass::WasJustResized() { ZoneScoped; }

void PlanetModule_Atmosphere_Mesh_Pass::DrawModel(vk::CommandBuffer* vCmdBuffer, const int& vIterationNumber) {
    ZoneScoped;
...
void PlanetModule_Atmosphere_Mesh_Pass::CreateCube() {
...
size_t PlanetModule_Atmosphere_Mesh_Pass::GetMiddlePoint_Plane(const size_t& p1, const size_t& p2, std::vector<VertexStruct::P3_N3_C4>& vVertices, CacheDB& vCache) {
    if (p1 == p2) {
        CTOOL_DEBUG_BREAK;
    }
...
void PlanetModule_Atmosphere_Mesh_Pass::CalcNormal(VertexStruct::P3_N3_C4& v0, VertexStruct::P3_N3_C4& v1, VertexStruct::P3_N3_C4& v2) {
...
void PlanetModule_Atmosphere_Mesh_Pass::Subdivide(const size_t& vSubdivLevel, std::vector<VertexStruct::P3_N3_C4>& vVertices, std::vector<TriFace>& vFaces) {
    if (vSubdivLevel > 0) {
...
void PlanetModule_Atmosphere_Mesh_Pass::BuildMesh() {
...
bool PlanetModule_Atmosphere_Mesh_Pass::CreateUBO() {
    ZoneScoped;

    m_UBO_Vert_Ptr = VulkanRessource::createUniformBufferObject(m_VulkanCore, sizeof(UBO_Vert), "PlanetModule_Atmosphere_Mesh_Pass");
    ...
    m_UBO_Tess_Ctrl_Ptr = VulkanRessource::createUniformBufferObject(m_VulkanCore, sizeof(UBO_Tess_Ctrl), "PlanetModule_Atmosphere_Mesh_Pass");
    ...
    m_UBO_Tess_Eval_Ptr = VulkanRessource::createUniformBufferObject(m_VulkanCore, sizeof(UBO_Tess_Eval), "PlanetModule_Atmosphere_Mesh_Pass");
    ...
    m_UBO_Frag_Ptr = VulkanRessource::createUniformBufferObject(m_VulkanCore, sizeof(UBO_Frag), "PlanetModule_Atmosphere_Mesh_Pass");
...
void PlanetModule_Atmosphere_Mesh_Pass::UploadUBO() {
    ZoneScoped;
...
void PlanetModule_Atmosphere_Mesh_Pass::DestroyUBO() {
    ZoneScoped;
...
bool PlanetModule_Atmosphere_Mesh_Pass::UpdateLayoutBindingInRessourceDescriptor() {
    ZoneScoped;
...
bool PlanetModule_Atmosphere_Mesh_Pass::UpdateBufferInfoInRessourceDescriptor() {
    ZoneScoped;
...
std::string PlanetModule_Atmosphere_Mesh_Pass::GetCommonCode() {
    return u8R"(
...
std::string PlanetModule_Atmosphere_Mesh_Pass::GetVertexShaderCode(std::string& vOutShaderName) {
    vOutShaderName = "PlanetModule_Atmosphere_Mesh_Pass_Vertex";
...
std::string PlanetModule_Atmosphere_Mesh_Pass::GetTesselationControlShaderCode(std::string& vOutShaderName) {
    vOutShaderName = "PlanetModule_Atmosphere_Mesh_Pass_Tesselation_Control";
...
std::string PlanetModule_Atmosphere_Mesh_Pass::GetTesselationEvaluationShaderCode(std::string& vOutShaderName) {
    vOutShaderName = "PlanetModule_Atmosphere_Mesh_Pass_Tesselation_Evaluation";
...
std::string PlanetModule_Atmosphere_Mesh_Pass::GetFragmentShaderCode(std::string& vOutShaderName) {
    vOutShaderName = "PlanetModule_Atmosphere_Mesh_Pass_Fragment";
...
std::string PlanetModule_Atmosphere_Mesh_Pass::getXml(const std::string& vOffset, const std::string& vUserDatas) {
    ZoneScoped;
    ...
    str += vOffset + "<planet_module_atmosphere_pass>\n";
...
bool PlanetModule_Atmosphere_Mesh_Pass::setFromXml(tinyxml2::XMLElement* vElem, tinyxml2::XMLElement* vParent, const std::string& vUserDatas) {
    ZoneScoped;
    ...
    if (strParentName == "planet_module_atmosphere_pass") {
...
void PlanetModule_Atmosphere_Mesh_Pass::AfterNodeXmlLoading() {
    ZoneScoped;
