...
bool PlanetModule_Ground_Mesh_Pass::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContext)
{
	...
	if (ImGui::CollapsingHeader("Mesh Analyze"))
	{
		need_ubos_update |= ImGui::CheckBoxIntDefault("Use debug", &m_UBO_Frag.u_use_debug, 0);
		if (m_UBO_Frag.u_use_debug)
		{
			...
			need_ubos_update |= ImGui::ContrastedComboVectorDefault(0.0f, "Channel", &m_UBO_Frag.u_show_layer, m_Channels, 0);
		...
		{
			need_ubos_update |= ImGui::CheckBoxIntDefault("Show Shaded Wireframe", &m_UBO_Frag.u_show_shaded_wireframe, 0);
		}

		if (!m_Vertices.m_Array.empty() && !m_Indices.m_Array.empty())
		{
			need_ubos_update |= ImGui::CheckBoxBoolDefault("Use Indices Restriction", &m_UseIndiceRestriction, false);
			if (m_UseIndiceRestriction)
			{
				need_ubos_update |= ImGui::SliderUIntDefaultCompact(0.0f, "count indices", &m_RestrictedIndicesCountToDraw, 0, (uint32_t)m_Indices.m_Array.size(), (uint32_t)m_Indices.m_Array.size());
			}
		}
	...
	{
		need_model_update |= ImGui::SliderUIntDefaultCompact(0.0f, "Subdivision Level", &m_SubdivisionLevel, 0U, 5U, 0U);
	}

	if (ImGui::CollapsingHeader("Mesh Tesselation"))
	{
		need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Radius", &m_UBO_Tess_Eval.u_radius, 0.0f, 10.0f, 1.0f);
		need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Tesselation level", &m_UBO_Tess_Ctrl.u_tesselation_level, 0.0f, 100.0f, 1.0f);
		need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Displacement Factor", &m_UBO_Tess_Eval.u_displace_factor, 0.0f, 2.0f, 1.0f);
		need_ubos_update |= ImGui::SliderFloatDefaultCompact(0.0f, "Normal PRecision", &m_UBO_Tess_Eval.u_normal_prec, 0.0f, 0.1f, 0.01f);
	}
