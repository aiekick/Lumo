...
bool DepthToPosNode::ExecuteAllTime(const uint32_t& vCurrentFrame, vk::CommandBuffer* vCmd, BaseNodeState* vBaseNodeState)
{
	...
	if (m_DepthToPosModulePtr)
	{
		return m_DepthToPosModulePtr->Execute(vCurrentFrame, vCmd, vBaseNodeState);
	}
	return false;
}

bool DepthToPosNode::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, const std::string& vUserDatas)
{
	assert(vContextPtr); ImGui::SetCurrentContext(vContextPtr);

	if (m_DepthToPosModulePtr)
	{
		return m_DepthToPosModulePtr->DrawWidgets(vCurrentFrame, vContextPtr, vUserDatas);
	}
...
bool DepthToPosNode::DrawDialogsAndPopups(
	...
	if (m_DepthToPosModulePtr)
	{
        return m_DepthToPosModulePtr->DrawDialogsAndPopups(vCurrentFrame, vMaxSize, vContextPtr, vUserDatas);
    }
...
void DepthToPosNode::NeedResizeByResizeEvent(ct::ivec2* vNewSize, const uint32_t* vCountColorBuffers)
{
	if (m_DepthToPosModulePtr)
	{
		m_DepthToPosModulePtr->NeedResizeByResizeEvent(vNewSize, vCountColorBuffers);
	}
...
void DepthToPosNode::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize)
{
	if (m_DepthToPosModulePtr)
	{
		m_DepthToPosModulePtr->SetTexture(vBindingPoint, vImageInfo, vTextureSize);
	}
}
