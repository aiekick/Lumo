...
bool Texture2DNode::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContext, const std::string& vUserDatas) {
    assert(vContext);
    ImGui::SetCurrentContext(vContext);

    if (m_Texture2DModule) {
        return m_Texture2DModule->DrawWidgets(vCurrentFrame, vContext, vUserDatas);
    }
...
bool Texture2DNode::DrawDialogsAndPopups(
    ...
    if (m_Texture2DModule) {
        return m_Texture2DModule->DrawDialogsAndPopups(vCurrentFrame, vMaxSize, vContext, vUserDatas);
    }
    return false;
}
