...
std::shared_ptr<FlatGradientModule> FlatGradientModule::Create(vkApi::VulkanCorePtr vVulkanCorePtr, BaseNodeWeak vParentNode)
{
	ZoneScoped;
...
FlatGradientModule::FlatGradientModule(vkApi::VulkanCorePtr vVulkanCorePtr)
...
bool FlatGradientModule::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContext)
{
	ZoneScoped;
	...
	if (m_LastExecutedFrame == vCurrentFrame)
	{
		if (ImGui::CollapsingHeader_CheckBox("Flat Gradient##FlatGradientModule", -1.0f, true, true, &m_CanWeRender))
		{
			...
			if (m_FlatGradientModule_Comp_2D_Pass_Ptr)
			{
				...
				change |= m_FlatGradientModule_Comp_2D_Pass_Ptr->DrawWidgets(vCurrentFrame, vContext);
			}
...
void FlatGradientModule::DrawOverlays(const uint32_t& vCurrentFrame, const ct::frect& vRect, ImGuiContext* vContext)
{
	ZoneScoped;
...
void FlatGradientModule::DisplayDialogsAndPopups(const uint32_t& vCurrentFrame, const ct::ivec2& vMaxSize, ImGuiContext* vContext)
{
	ZoneScoped;
...
void FlatGradientModule::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize)
{	
	ZoneScoped;

	if (m_FlatGradientModule_Comp_2D_Pass_Ptr)
	{
		m_FlatGradientModule_Comp_2D_Pass_Ptr->SetTexture(vBindingPoint, vImageInfo, vTextureSize);
	}
}
...
vk::DescriptorImageInfo* FlatGradientModule::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize)
{	
	ZoneScoped;

	if (m_FlatGradientModule_Comp_2D_Pass_Ptr)
	{
		return m_FlatGradientModule_Comp_2D_Pass_Ptr->GetDescriptorImageInfo(vBindingPoint, vOutSize);
	}
