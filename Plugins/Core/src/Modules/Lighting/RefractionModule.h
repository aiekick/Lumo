...
{
public:
	static std::shared_ptr<RefractionModule> Create(vkApi::VulkanCorePtr vVulkanCorePtr, BaseNodeWeak vParentNode);

private:
	ct::cWeak<RefractionModule> m_This;

	std::shared_ptr<Refraction_Quad_Pass> m_Refraction_Quad_Pass_Ptr = nullptr;

public:
	RefractionModule(vkApi::VulkanCorePtr vVulkanCorePtr);
