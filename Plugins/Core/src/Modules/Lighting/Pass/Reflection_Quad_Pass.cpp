...
Reflection_Quad_Pass::Reflection_Quad_Pass(GaiApi::VulkanCorePtr vVulkanCorePtr)
...
bool Reflection_Quad_Pass::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContext, const std::string& vUserDatas)
{
	assert(vContext); 
...
bool Reflection_Quad_Pass::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContext, const std::string& vUserDatas) {
    ZoneScoped;
	assert(vContext); 
	ImGui::SetCurrentContext(vContext);
    return false;
...
bool Reflection_Quad_Pass::DrawDialogsAndPopups(const uint32_t& vCurrentFrame, const ImVec2& vMaxSize, ImGuiContext* vContext, const std::string& vUserDatas) {
    ZoneScoped;
	assert(vContext); 
	ImGui::SetCurrentContext(vContext);
    return false;
