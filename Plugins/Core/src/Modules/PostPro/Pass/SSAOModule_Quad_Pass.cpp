...
SSAOModule_Quad_Pass::~SSAOModule_Quad_Pass()
{
	Unit();
}
...
void SSAOModule_Quad_Pass::SetTexture(const uint32_t& vBinding, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize)
{
	ZoneScoped;
...
vk::DescriptorImageInfo* SSAOModule_Quad_Pass::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize)
{
	if (m_FrameBufferPtr)
	{
		if (vOutSize)
...
bool SSAOModule_Quad_Pass::CreateUBO()
{
	ZoneScoped;
...
void SSAOModule_Quad_Pass::UploadUBO()
{
	ZoneScoped;
...
void SSAOModule_Quad_Pass::DestroyUBO()
{
	ZoneScoped;
...
bool SSAOModule_Quad_Pass::UpdateLayoutBindingInRessourceDescriptor()
{
	ZoneScoped;
...
bool SSAOModule_Quad_Pass::UpdateBufferInfoInRessourceDescriptor()
{
	ZoneScoped;
	...
	m_DescriptorSets[0].m_WriteDescriptorSets.emplace_back(m_DescriptorSets[0].m_DescriptorSet, 2U, 0, 1, vk::DescriptorType::eCombinedImageSampler, &m_ImageInfos[0], nullptr); // depth
	...
	m_DescriptorSets[0].m_WriteDescriptorSets.emplace_back(m_DescriptorSets[0].m_DescriptorSet, 4U, 0, 1, vk::DescriptorType::eCombinedImageSampler, &m_ImageInfos[2], nullptr); // RGB noise

	return true;
}

std::string SSAOModule_Quad_Pass::GetVertexShaderCode(std::string& vOutShaderName)
{
...
std::string SSAOModule_Quad_Pass::GetFragmentShaderCode(std::string& vOutShaderName)
{
	vOutShaderName = "SSAOModule_Quad_Pass";
...
std::string SSAOModule_Quad_Pass::getXml(const std::string& vOffset, const std::string& /*vUserDatas*/)
{
	std::string str;
...
bool SSAOModule_Quad_Pass::setFromXml(tinyxml2::XMLElement* vElem, tinyxml2::XMLElement* vParent, const std::string& /*vUserDatas*/)
{
