...
std::vector<LibraryEntry> PostProcessing::GetLibrary() const
{
    ...
    res.push_back(AddLibraryEntry("3D/PostPro", "SSAO", "SSAO"));
	res.push_back(AddLibraryEntry("2D/PostPro", "Blur", "BLUR"));
	res.push_back(AddLibraryEntry("2D/PostPro", "Bloom", "BLOOM"));
	res.push_back(AddLibraryEntry("2D/PostPro", "Tone Map", "TONE_MAP"));
	res.push_back(AddLibraryEntry("2D/PostPro", "Vignette", "VIGNETTE"));
...
BaseNodePtr PostProcessing::CreatePluginNode(const std::string& vPluginNodeName)
{
	...
	// Post Processing
	if (vPluginNodeName == "SSAO")				return SSAONode::Create(vkPostProcessingPtr);
	else if (vPluginNodeName == "BLUR")			return BlurNode::Create(vkPostProcessingPtr);
	else if (vPluginNodeName == "BLOOM")		return BloomNode::Create(vkPostProcessingPtr);
	else if (vPluginNodeName == "TONE_MAP")		return ToneMapNode::Create(vkPostProcessingPtr);
	else if (vPluginNodeName == "VIGNETTE")		return VignetteNode::Create(vkPostProcessingPtr);
