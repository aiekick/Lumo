...
{
public:
	static std::shared_ptr<DepthofFieldModule_Comp_2D_Pass> Create(const ct::uvec2& vSize, GaiApi::VulkanCorePtr vVulkanCorePtr);

private:
	struct UBO_Comp {
		alignas(4) float u_backgroud_blur_amount = 1.0f;
		alignas(4) float u_foreground_blur_amount = 1.0f;
...
public:
	DepthofFieldModule_Comp_2D_Pass(GaiApi::VulkanCorePtr vVulkanCorePtr);
	~DepthofFieldModule_Comp_2D_Pass() override;
