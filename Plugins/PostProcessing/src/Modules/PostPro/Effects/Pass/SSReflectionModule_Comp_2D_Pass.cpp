...
bool SSReflectionModule_Comp_2D_Pass::UpdateLayoutBindingInRessourceDescriptor()
{
	...
	res &= AddOrSetLayoutDescriptor(4U, vk::DescriptorType::eStorageImage, vk::ShaderStageFlagBits::eCompute); // color output 
...
bool SSReflectionModule_Comp_2D_Pass::UpdateBufferInfoInRessourceDescriptor()
{
	...
	res &= AddOrSetWriteDescriptorImage(2U, vk::DescriptorType::eCombinedImageSampler, &m_ImageInfos[1U]);  // sampler input
	res &= AddOrSetWriteDescriptorImage(3U, vk::DescriptorType::eCombinedImageSampler, &m_ImageInfos[2U]);  // sampler input
	res &= AddOrSetWriteDescriptorImage(4U, vk::DescriptorType::eStorageImage, m_ComputeBufferPtr->GetFrontDescriptorImageInfo(0U)); // output
...
layout(binding = 2) uniform sampler2D input_Position_map;
layout(binding = 3) uniform sampler2D input_Normal_map;

layout(binding = 4, rgba32f) uniform image2D colorBuffer; // output
