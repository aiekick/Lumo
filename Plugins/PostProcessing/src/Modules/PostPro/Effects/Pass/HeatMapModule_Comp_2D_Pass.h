...
{
public:
	static std::shared_ptr<HeatMapModule_Comp_2D_Pass> Create(const ct::uvec2& vSize, GaiApi::VulkanCoreWeak vVulkanCore);

private:
    struct UBO_Comp {
        alignas(4) uint32_t u_channel_idx = 0U;
        alignas(4) uint32_t u_count_colors = 0U;
        alignas(4) float u_min = 0.0f;
        alignas(4) float u_max = 1.0f;
...
public:
	HeatMapModule_Comp_2D_Pass(GaiApi::VulkanCoreWeak vVulkanCore);
	~HeatMapModule_Comp_2D_Pass() override;
