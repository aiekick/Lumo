...
bool VignetteModule::Init()
{
	...
	if (BaseRenderer::InitCompute2D(map_size))
	{
		...
		m_VignetteModule_Comp_2D_Pass_Ptr = std::make_shared<VignetteModule_Comp_2D_Pass>(m_VulkanCorePtr);
		if (m_VignetteModule_Comp_2D_Pass_Ptr)
		{
			// by default but can be changed via widget
			m_VignetteModule_Comp_2D_Pass_Ptr->AllowResizeOnResizeEvents(false);
			m_VignetteModule_Comp_2D_Pass_Ptr->AllowResizeByHandOrByInputs(true);
...
bool VignetteModule::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, const std::string& vUserDatas)
{
	...
	{
		if (ImGui::CollapsingHeader_CheckBox("Vignette##VignetteModule", -1.0f, true, true, &m_CanWeRender))
		{
			bool change = false;

			if (m_VignetteModule_Comp_2D_Pass_Ptr)
			{
				change |= m_VignetteModule_Comp_2D_Pass_Ptr->DrawWidgets(vCurrentFrame, vContextPtr, vUserDatas);
			}
...
void VignetteModule::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize)
{	
	ZoneScoped;
	if (m_VignetteModule_Comp_2D_Pass_Ptr)
	{
		m_VignetteModule_Comp_2D_Pass_Ptr->SetTexture(vBindingPoint, vImageInfo, vTextureSize);
	}
}
...
vk::DescriptorImageInfo* VignetteModule::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize)
{	
	ZoneScoped;
	if (m_VignetteModule_Comp_2D_Pass_Ptr)
	{
