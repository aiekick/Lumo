...
{
public:
	static std::shared_ptr<ShapeModule_Comp_3D_Pass> Create(const ct::uvec3& vSize, GaiApi::VulkanCoreWeak vVulkanCore);

private:
	struct UBO_Comp {
		alignas(4) int32_t u_type = 0; // cube, sphere, torus;
		alignas(4) float u_size = 1.0f;
		alignas(4) float u_thick = 0.0f;
		alignas(16) ct::fvec4 u_color = ct::fvec4(0.0f, .0f, .0f, .0f);
		alignas(4) float u_Transform = 0.0f;
	} m_UBO_Comp;
...
public:
	ShapeModule_Comp_3D_Pass(GaiApi::VulkanCoreWeak vVulkanCore);
	~ShapeModule_Comp_3D_Pass() override;
