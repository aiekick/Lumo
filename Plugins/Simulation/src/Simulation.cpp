...
std::vector<LibraryEntry> Simulation::GetLibrary() const {
    ...
    res.push_back(AddLibraryEntry("Differential Ops", "Laplacian", "LAPLACIAN"));
    res.push_back(AddLibraryEntry("Differential Ops", "Divergence", "DIVERGENCE"));
    res.push_back(AddLibraryEntry("Differential Ops", "Gradient", "GRADIENT"));
    res.push_back(AddLibraryEntry("Differential Ops", "Curl (Rotational)", "CURL"));

    res.push_back(AddLibraryEntry("Simulation", "GrayScott", "2D_SIMULATION_GRAY_SCOTT"));
    res.push_back(AddLibraryEntry("Simulation", "Conway", "2D_SIMULATION_CONWAY"));
...
BaseNodePtr Simulation::CreatePluginNode(const std::string& vPluginNodeName) {
    ...
    if (vPluginNodeName == "LAPLACIAN")
        return LaplacianNode::Create(vkCorePtr);
    else if (vPluginNodeName == "DIVERGENCE")
        return DivergenceNode::Create(vkCorePtr);
    else if (vPluginNodeName == "GRADIENT")
        return GradientNode::Create(vkCorePtr);
    else if (vPluginNodeName == "CURL")
        return CurlNode::Create(vkCorePtr);

    // Simulations
    else if (vPluginNodeName == "2D_SIMULATION_GRAY_SCOTT")
        return GrayScottNode::Create(vkCorePtr);
    else if (vPluginNodeName == "2D_SIMULATION_CONWAY")
        return ConwayNode::Create(vkCorePtr);
