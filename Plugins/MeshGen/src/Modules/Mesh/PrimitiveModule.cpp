...
bool PrimitiveModule::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    ZoneScoped;
...
bool PrimitiveModule::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    ZoneScoped;
    assert(vContextPtr);
    ImGui::SetCurrentContext(vContextPtr);
    return false;
    ...
    if (ImGui::CollapsingHeader("Primitives")) {
        ...
        switch (m_PrimitiveTypeIndex) {
            ...
            case PRIMITIVE_TYPE_TORUS: {
                // tofix : desactiv� pour le moment vu que ca amrche pas
...
void PrimitiveModule::prUpdateMesh() {
    ...
    switch (m_PrimitiveTypeIndex) {
        case PRIMITIVE_TYPE_PLANE: CreatePlane(); break;
        case PRIMITIVE_TYPE_CUBE: CreateCube(); break;
        case PRIMITIVE_TYPE_ICO_SPHERE: CreateIcosaedre(); break;
        case PRIMITIVE_TYPE_UV_SPHERE: CreateUVSphere(); break;
        case PRIMITIVE_TYPE_TORUS: CreateTorus(); break;
        case PRIMITIVE_TYPE_FIBONACCI_BALL: CreateFibonacciBall(); break;
        case PRIMITIVE_TYPE_Count:
        default: {
            CTOOL_DEBUG_BREAK;
        } break;
...
void PrimitiveModule::CreatePlane() {
    ...
    std::vector<ct::fvec3> plane_points = {ct::fvec3(-1, 0, -1), ct::fvec3(1, 0, -1), ct::fvec3(1, 0, 1), ct::fvec3(-1, 0, 1)};

    std::vector<ct::fvec2> plane_uvs = {ct::fvec2(0, 0), ct::fvec2(1, 0), ct::fvec2(1, 1), ct::fvec2(0, 1)};
...
void PrimitiveModule::CreateCube() {
    if (m_GenerateUVs) {
        ...
        for (size_t point_idx = 0; point_idx < cube_points.size(); ++point_idx) {
            ...
            m_Vertices.emplace_back(cube_points.at(point_idx) * mid_size, cube_normals.at(normal_idx), ct::fvec3(), ct::fvec3(), cube_uvs.at(uv_idx));
        }
    ...
    } else  // no uvs, so all vertices are merged
    {
        ...
        */
        ...
        for (size_t point_idx = 0; point_idx < cube_points.size(); ++point_idx) {
            ...
            m_Vertices.emplace_back(point, normal, ct::fvec3(), ct::fvec3(), ct::fvec2());
        }
    ...
    if (!m_Vertices.empty()) {
        ...
        if (!m_QuadFaces.empty()) {
            ...
            for (const auto& f : m_QuadFaces) {
                ...
                CalcNormal(vertices.at(f.v0), vertices.at(f.v2), vertices.at(f.v3));
            }
        }
