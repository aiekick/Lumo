...
bool ParametricSurfaceUVModule::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, const std::string& vUserDatas)
{
	ZoneScoped;
	assert(vContextPtr); 
...
void ParametricSurfaceUVModule::SetVariable(const uint32_t& vVarIndex, SceneVariableWeak vSceneVariable)
{	
	ZoneScoped;
...
void ParametricSurfaceUVModule::prUpdateMesh()
{
	...
	if (expr_x && expr_y && expr_z)
	{
		...
		if (IS_DOUBLE_EQUAL(m_Step_U, 0.0)) return;
		if (IS_DOUBLE_EQUAL(m_Step_V, 0.0)) return;

		int verts_len_U = (int)ceil((m_End_U - m_Start_U) / m_Step_U);
		if (!verts_len_U) return;

		int verts_len_V = (int)ceil((m_End_V - m_Start_V) / m_Step_V);
		if (!verts_len_V) return;
		...
		auto& u_value = m_VarNameValues.at("u"); u_value = (double)m_Start_U;
		auto& v_value = m_VarNameValues.at("v"); v_value = (double)m_Start_V;
		...
		for (int verts_index_v = 0; verts_index_v < verts_len_V; ++verts_index_v)
		{
			...
			for (int verts_index_u = 0; verts_index_u < verts_len_U; ++verts_index_u)
			{
				...
				if (m_CloseU && (verts_index_u == polys_len_u) || 
					m_CloseV && (verts_index_v == polys_len_v))
					continue;
