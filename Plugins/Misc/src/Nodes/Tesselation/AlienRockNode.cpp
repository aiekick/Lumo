...
std::shared_ptr<AlienRockNode> AlienRockNode::Create(GaiApi::VulkanCoreWeak vVulkanCore) {
    ZoneScoped;
...
bool AlienRockNode::Init(GaiApi::VulkanCoreWeak vVulkanCore) {
    ZoneScoped;
...
bool AlienRockNode::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    ZoneScoped;
    ...
    if (m_AlienRockModulePtr) {
        res = m_AlienRockModulePtr->DrawWidgets(vCurrentFrame, vContextPtr, vUserDatas);
    }

    return res;
}

bool AlienRockNode::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, const std::string& vUserDatas) {
    return false;
}

bool AlienRockNode::DrawDialogsAndPopups(
    ...
    if (m_AlienRockModulePtr) {
        return m_AlienRockModulePtr->DrawDialogsAndPopups(vCurrentFrame, vMaxSize, vContextPtr, vUserDatas);
    }
    return false;
}
