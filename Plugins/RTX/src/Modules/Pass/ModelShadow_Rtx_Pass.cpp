...
bool ModelShadow_Rtx_Pass::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContext)
{
	...
	change |= ImGui::CheckBoxFloatDefault("Enable Light Attenuation", 
		&m_UBO_Rtx.u_enable_light_attenuation, m_Default_UBO_Rtx.u_enable_light_attenuation);
	if (m_UBO_Rtx.u_enable_light_attenuation > 0.5f)
	{
		change |= ImGui::SliderFloatDefaultCompact(0.0f, "Attenuation Factor", 
			&m_UBO_Rtx.u_light_intensity_factor, 0.0f, 1000.0f, m_Default_UBO_Rtx.u_light_intensity_factor);
	}
	change |= ImGui::CheckBoxFloatDefault("Enable Light Color", 
		&m_UBO_Rtx.u_enable_light_color, m_Default_UBO_Rtx.u_enable_light_color); 
	change |= ImGui::SliderFloatDefaultCompact(0.0f, "Shadow Strength",
			&m_UBO_Rtx.u_shadow_strength, 0.0f, 1.0f, m_Default_UBO_Rtx.u_shadow_strength);
...
std::string ModelShadow_Rtx_Pass::getXml(const std::string& vOffset, const std::string& vUserDatas)
{
	...
	str += vOffset + "<enable_light_attenuation>" + (m_UBO_Rtx.u_enable_light_attenuation > 0.5f ? "true" : "false") + "</enable_light_attenuation>\n";
