...
ShadowMapModule_Mesh_Pass::ShadowMapModule_Mesh_Pass(GaiApi::VulkanCoreWeak vVulkanCore) : ShaderPass(vVulkanCore) {
...
void ShadowMapModule_Mesh_Pass::DrawModel(vk::CommandBuffer* vCmdBufferPtr, const int& vIterationNumber) {
    ...
    if (vCmdBufferPtr && m_PushConstants.light_id_to_use < SceneLightGroup::sMaxLightCount) {
        ...
        vCmdBufferPtr->bindPipeline(vk::PipelineBindPoint::eGraphics, m_Pipelines[0].m_Pipeline);
        {
            ...
            vCmdBufferPtr->pushConstants(m_Pipelines[0].m_PipelineLayout, vk::ShaderStageFlagBits::eVertex | vk::ShaderStageFlagBits::eFragment, 0,
...
bool ShadowMapModule_Mesh_Pass::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    assert(vContextPtr);
...
bool ShadowMapModule_Mesh_Pass::DrawOverlays(
...
bool ShadowMapModule_Mesh_Pass::DrawDialogsAndPopups(
