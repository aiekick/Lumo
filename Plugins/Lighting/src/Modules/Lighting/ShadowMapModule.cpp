...
ShadowMapModule::ShadowMapModule(GaiApi::VulkanCoreWeak vVulkanCore) : BaseRenderer(vVulkanCore) {
}
        ...
        for (size_t i = 0U; i < 8U; ++i) {
            ...
            if (m_FrameBuffers[i]) {
                if (i == 0U) {
                    ...
                    res &= m_FrameBuffers[i]->Init(1024U, 1U, true, true, 0.0f, false, vk::Format::eR32G32B32A32Sfloat, vk::SampleCountFlagBits::e1);
                } else {
                    auto renderpassPtr = m_FrameBuffers[0]->GetRenderPass();
                    if (renderpassPtr) {
                        res &= m_FrameBuffers[i]->Init(
                            1024U, 1U, true, true, 0.0f, false, vk::Format::eR32G32B32A32Sfloat, vk::SampleCountFlagBits::e1, false, *renderpassPtr);
                    }
                }
            }
        }
        ...
        if (res) {
            ...
            if (m_ShadowMapModule_Mesh_Pass_Ptr) {
                if (m_ShadowMapModule_Mesh_Pass_Ptr->InitPixelWithoutFBO(
                        1024U, 1U, false, m_FrameBuffers[0]->GetRenderPass(), vk::SampleCountFlagBits::e1)) {
...
bool ShadowMapModule::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    assert(vContextPtr);
...
bool ShadowMapModule::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, void* vUserDatas) {
    assert(vContextPtr);
    ImGui::SetCurrentContext(vContextPtr);
...
bool ShadowMapModule::DrawDialogsAndPopups(
