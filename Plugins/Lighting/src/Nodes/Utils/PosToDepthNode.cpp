...
std::shared_ptr<PosToDepthNode> PosToDepthNode::Create(GaiApi::VulkanCoreWeak vVulkanCore) {
...
bool PosToDepthNode::ExecuteAllTime(const uint32_t& vCurrentFrame, vk::CommandBuffer* vCmd, BaseNodeState* vBaseNodeState) {
...
bool PosToDepthNode::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    ...
    if (m_PosToDepthModulePtr) {
        return m_PosToDepthModulePtr->DrawWidgets(vCurrentFrame, vContextPtr, vUserDatas);
    }
...
bool PosToDepthNode::DrawDialogsAndPopups(
    ...
    if (m_PosToDepthModulePtr) {
        return m_PosToDepthModulePtr->DrawDialogsAndPopups(vCurrentFrame, vMaxRect, vContextPtr, vUserDatas);
    }

    return false;
}

void PosToDepthNode::DisplayInfosOnTopOfTheNode(BaseNodeState* vBaseNodeState) {
...
void PosToDepthNode::NeedResizeByResizeEvent(ct::ivec2* vNewSize, const uint32_t* vCountColorBuffers) {
    if (m_PosToDepthModulePtr) {
        m_PosToDepthModulePtr->NeedResizeByResizeEvent(vNewSize, vCountColorBuffers);
    }
...
void PosToDepthNode::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize, void* vUserDatas) {
    if (m_PosToDepthModulePtr) {
        m_PosToDepthModulePtr->SetTexture(vBindingPoint, vImageInfo, vTextureSize, vUserDatas);
    }
}

vk::DescriptorImageInfo* PosToDepthNode::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize, void* vUserDatas) {
    if (m_PosToDepthModulePtr) {
        return m_PosToDepthModulePtr->GetDescriptorImageInfo(vBindingPoint, vOutSize, vUserDatas);
    }
