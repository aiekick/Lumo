...
bool ParametricCurveDiffNode::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    ZoneScoped;
    assert(vContextPtr);
...
bool ParametricCurveDiffNode::DrawDialogsAndPopups(
    ...
    ImGui::SetCurrentContext(vContextPtr);
    if (m_ParametricCurveDiffModulePtr) {
        return m_ParametricCurveDiffModulePtr->DrawDialogsAndPopups(vCurrentFrame, vMaxRect, vContextPtr, vUserDatas);
    }
    return false;
}
...
void ParametricCurveDiffNode::SetVariable(const uint32_t& vVarIndex, SceneVariableWeak vSceneVariable, void* vUserDatas) {
    ZoneScoped;

    if (m_ParametricCurveDiffModulePtr) {
        m_ParametricCurveDiffModulePtr->SetVariable(vVarIndex, vSceneVariable, nullptr);
    }
}
