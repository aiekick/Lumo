...
BillBoardRendererModule::BillBoardRendererModule(GaiApi::VulkanCoreWeak vVulkanCore) : TaskRenderer(vVulkanCore) {
    ZoneScoped;
...
bool BillBoardRendererModule::Init() {
    ...
    if (TaskRenderer::InitPixel(map_size)) {
        ...
        if (m_BillBoardRendererModule_Mesh_Pass_Ptr) {
            ...
            if (m_BillBoardRendererModule_Mesh_Pass_Ptr->InitPixel(
                    map_size, 1U, false, true, 0.0f, false, false, vk::Format::eR32G32B32A32Sfloat, vk::SampleCountFlagBits::e2)) {
...
bool BillBoardRendererModule::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    ZoneScoped;
...
void BillBoardRendererModule::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ct::fvec2* vTextureSize, void* vUserDatas) {
    ZoneScoped;

    if (m_BillBoardRendererModule_Mesh_Pass_Ptr) {
        m_BillBoardRendererModule_Mesh_Pass_Ptr->SetTexture(vBindingPoint, vImageInfo, vTextureSize, vUserDatas);
    }
}
...
vk::DescriptorImageInfo* BillBoardRendererModule::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ct::fvec2* vOutSize, void* vUserDatas) {
    ZoneScoped;

    if (m_BillBoardRendererModule_Mesh_Pass_Ptr) {
        return m_BillBoardRendererModule_Mesh_Pass_Ptr->GetDescriptorImageInfo(vBindingPoint, vOutSize, vUserDatas);
    }
