...
void NodeSlot%sInput::Unit()
{
	...
	// car NodeSlot%sInput peut etre isntanci� � l'ancienne en copie local donc sans shared_ptr
	// donc pour gagner du temps on va checker le this, si expir� on va pas plus loins
	if (!m_This.expired())
	{
...
{
	// ici pas besoin du assert sur le m_This 
	// car NodeSlot%sOutput peut etre instanci� � l'ancienne en copie local donc sans shared_ptr
	// donc pour gagner du temps on va checker le this, si expir� on va pas plus loins
	if (!m_This.expired())
	{
		if (!parentNode.expired())
