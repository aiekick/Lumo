...
bool DilationNode::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    ZoneScoped;
    bool res = false;
    assert(vContextPtr);
...
bool DilationNode::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, void* vUserDatas) {
    ZoneScoped;
...
bool DilationNode::DrawDialogsAndPopups(
    ...
    ImGui::SetCurrentContext(vContextPtr);
    if (m_DilationModulePtr) {
        return m_DilationModulePtr->DrawDialogsAndPopups(vCurrentFrame, vMaxRect, vContextPtr, vUserDatas);
    }
    return false;
}
